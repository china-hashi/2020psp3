...
l.97: iがcn-1より小さい間、以下の処理を繰り返す

l.98: pPrevにpPrev->pNextを代入(削除したい要素の1つ前を探す)

l.104: 削除したい要素のメモリを解放する

l.108: pNodeがNULLとなるまで以下の処理を繰り返す

l.113: もしiが20なら以下の処理をする


l.135: Node型のポインタpSearchにリストの先頭アドレスpListを代入する

l.137: pSearchがNULLとなるまで以下の処理を繰り返す

l.138: もしリスト中のIDと検索するIDが一致したら以下の処理をする

l.139: pCityに一致したデータを代入する

l.140: iを返す

l.142: pSearchに次のリストの先頭アドレスを代入する

l.143: iに1を足す

l.145: 繰り返し処理終了後-1を返す
