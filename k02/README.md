...
l.97: もしcnが0ならば以下の処理をする

l.100: 削除したい要素のメモリを解放する

l.102: iがcn-1より小さい間、以下の処理を繰り返す

l.103: pPrevにpPrev->pNextを代入(削除したい要素の1つ前を探す)

l.108: 削除したい要素のメモリを解放する

l.113: pNodeがNULLとなるまで以下の処理を繰り返す

l.118: もしiが20なら以下の処理をする


l.140: Node型のポインタpSearchにリストの先頭アドレスpListを代入する

l.142: pSearchがNULLとなるまで以下の処理を繰り返す

l.143: もしリスト中のIDと検索するIDが一致したら以下の処理をする

l.144: pCityに一致したデータを代入する

l.145: iを返す

l.147: pSearchに次のリストの先頭アドレスを代入する

l.148: iに1を足す

l.150: 繰り返し処理終了後-1を返す
...
cn=0の時、つまり、ID42411を指定してShinkamigotoを削除する場合、うまく削除できていないようです。
ほかの場合はうまくいっていますのでその点を修正してみてください。